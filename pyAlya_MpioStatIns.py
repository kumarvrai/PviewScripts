...
for instant in listOfInstants:
	if instant%5 == 0: pyAlya.pprint(1,'First loop, instant %d...'%instant,flush=True)
	# Read field
	...
	stats['AVPRE'] += fields['PRESS']
	stats['AVVEL'] += fields['VELOC']

stats['AVPRE'] = stats['AVPRE']/len(listOfInstants)
stats['AVVEL'] = stats['AVVEL']/len(listOfInstants)
...
for instant in listOfInstants:
	...
	fields['GRVFL'] = pyAlya.math.linopArrf(1.,fields['GRADV'],-1.,stats['AVVGR'])
	fields['RESTR'] = rho*pyAlya.stats.reynoldsStressTensor(fields['VFLUC'])
...
stats['RESTR'] = stats['RESTR']/len(listOfInstants)
...
stats['PRODU'] = pyAlya.stats.productionBudget(stats['RESTR'],stats['AVVGR'])
...
if COMPUTE_EARSM:
	...
	stats['LAVEL'] = mesh.divergence(fields['AVVGR']) # lap(U) = div(grad(U))
...
writer.writeDataset('Inputs',
		     ...
		     # Gradients
			 mesh.filter_bc(stats['AVPGR'][:,0]), # px
			 mesh.filter_bc(stats['AVVGR'][:,0]), # ux
			 mesh.filter_bc(stats['AVVGR'][:,3]), # vx
			 mesh.filter_bc(stats['AVVGR'][:,6]), # wx
			 mesh.filter_bc(stats['AVPGR'][:,1]), # py
			 mesh.filter_bc(stats['AVVGR'][:,1]), # uy
			 mesh.filter_bc(stats['AVVGR'][:,4]), # vy
			 mesh.filter_bc(stats['AVVGR'][:,5]), # wy
			 mesh.filter_bc(stats['AVPGR'][:,2]), # pz
			 mesh.filter_bc(stats['AVVGR'][:,2]), # uz
			 mesh.filter_bc(stats['AVVGR'][:,5]), # vz
			 mesh.filter_bc(stats['AVVGR'][:,8])  # wz
)
