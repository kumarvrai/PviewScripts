...
ALT_BASEDIR    = ''
CASESTR        = 'chan'
VARLIST        = ['VELOC','PRESS', 'AVPRE', 'AVVEL', 'AVVE2', 'AVVXY']
#START, DT, END = 2,1,415
START, DT, END = 2000,2000,144000
...
exStats = pyAlya.Field(xyz  = pyAlya.truncate(mesh.xyz,6),
					# Level 1 - averaged Navier-Stokes equations
...
for instant in listOfInstants:
	...
	exStats['AVVE2'] += pyAlya.stats.addS1(exStats['AVVE2'],fields['AVVE2'],w=1. if instant == START else dt/time)
	exStats['AVVXY'] += pyAlya.stats.addS1(exStats['AVVXY'],fields['AVVXY'],w=1. if instant == START else dt/time)
	#stats['AVPRE'] += fields['AVPRE']
	#stats['AVVEL'] += fields['AVVEL']
...
stats['RESTR'][:,0] = exStats['AVVE2'][:,0]-stats['AVVEL'][:,0]**2
stats['RESTR'][:,4] = exStats['AVVE2'][:,1]-stats['AVVEL'][:,1]**2
stats['RESTR'][:,8] = exStats['AVVE2'][:,2]-stats['AVVEL'][:,2]**2
stats['RESTR'][:,1] = exStats['AVVXY'][:,0]-stats['AVVEL'][:,0]*stats['AVVEL'][:,1]
stats['RESTR'][:,2] = exStats['AVVXY'][:,1]-stats['AVVEL'][:,1]*stats['AVVEL'][:,2]
stats['RESTR'][:,5] = exStats['AVVXY'][:,2]-stats['AVVEL'][:,0]*stats['AVVEL'][:,2]
...
#	'AVTEM','AVHFL','AVSTR','AVROT','AVSHE','RESTR','AVSTF','AVRTF','AVTHF',
