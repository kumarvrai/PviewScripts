...
center = np.empty((0,2),dtype='float')
rS = []
# Roughness on Upper surface
for i in range(0,len(XposU)):
    xLoc = XposU[i]
    yLoc = F0(xLoc)
    center = np.vstack((center,np.array([xLoc,yLoc])))
...
for n in range(0,Roughness):
  print('----||R', n+1,'x0=',center[n,0],'y0=',center[n,1],'h = ',rS[n])
  if(center[n,1]>0.0 and rS[n]==h):
   rInd = n;
...
#                
                    ...
                    rad_loc = (x-xc0)**2.0/(aSphere)**2+(y-yc0)**2.0/(bSphere)**2
                    if rad_loc <= 1.0:
